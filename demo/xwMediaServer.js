;(function (name, definition) {
  ...
  if (hasDefine) {
	...
	var isSupported = function() {
  	...
  	var Log$1 = new Logger(DEFAULT_LOG_LEVEL, "XwRtspPlayer");
	...
	var XwRtspPlayer = function(url, video, config, evnetTrigger) {
		this._id = creIdFn();
		this._url = url;
		this._video = video;
		var meConfig = this._config = config || {};

		setConfig(meConfig, XwRtspPlayer.defConfig);
	...
	XwRtspPlayer.isSupported = isSupported();
	
	XwRtspPlayer.Events = EventNames;

	XwRtspPlayer.LogLevel = LogLevel;
	XwRtspPlayer.defConfig = defConfig;

	XwRtspPlayer.version = VERSION;

	XwRtspPlayer.prototype._id = null;
	XwRtspPlayer.prototype._isPlay = false;
	XwRtspPlayer.prototype._isInit = null;
	XwRtspPlayer.prototype._isCreAudio = false;

	XwRtspPlayer.prototype._video = null;
	XwRtspPlayer.prototype._audio = null;

	XwRtspPlayer.prototype._vMes = null;
	XwRtspPlayer.prototype._aMes = null;

	XwRtspPlayer.prototype._bufferDelay = 0.15; 		//延迟时间
	XwRtspPlayer.prototype._bufferDelayOffect = 0.3;	//延迟可忽略的时间
	XwRtspPlayer.prototype._setCurrentTimeInterval = 1000 * 10;

	XwRtspPlayer.prototype._bufferDuration = 125;
					
	XwRtspPlayer.prototype._keepaliveTime = 1000;
	XwRtspPlayer.prototype._keepaliveInterval = 500;
	
	XwRtspPlayer.prototype._isDestroyd = false;

	XwRtspPlayer.prototype._init = function() {
		if (!this._isInit) {
			this._isInit = true;
	...
	XwRtspPlayer.prototype._dispatchEvent = function(name, args) {
		var data = {
			id:this._id, 
			url : this._url
		};
		...
		Log$1.log("xwRtspPlayer[" + this._id + "] dispatchEvent " + name +":", data);
		dispatchEvent(this._evnetTrigger, name, data);
	}
			
	XwRtspPlayer.prototype._volumechange = function() {
	 ...
	 * 播放
	 */
	XwRtspPlayer.prototype.play = function() {
		if (this._isPlay) {
			return;
		}
		this._isPlay = true;
	    ...
	    ws.onopen = function (ev) {
	    	...
	    	Log$1.log("xwRtspPlayer[" + this._id + "] 连接成功");
	    }.bind(this);
	    ws.onerror = function (ev) {
	    	Log$1.error("xwRtspPlayer[" + this._id + "] 连接错误");
	    	this._dispatchEvent(EventNames.error);
	    }.bind(this);
	    ws.onclose = function (ev) {
	    	this._stopKeepAlive();
	    	Log$1.log("xwRtspPlayer[" + this._id + "] 连接关闭");
	    }.bind(this);
	 ...
	 * 开始心跳
	 */
	XwRtspPlayer.prototype._startKeepAlive = function() {
	 ...
	 * 停止心跳
	 */
	XwRtspPlayer.prototype._stopKeepAlive = function() {
	 ...
	 * 发送心跳
	 */
	XwRtspPlayer.prototype._KeepAlive = function() {
		if (this._ws) {
			this._ws.send("keep:\n");
		}
	}
	 ...
	 * 喂数据
	 */
	XwRtspPlayer.prototype._feed = function(data) {
		...
		if (udata[0] == 1 && udata[1] == 1 && udata[2] == 1 && udata[3] == 1) {
			...
			Log$1.error("xwRtspPlayer[" + this._id + "]" + dataString);
	 ...
	 * 停止
	 */
	XwRtspPlayer.prototype.stop = function() {
		this._isPlay = false;
		if (this._ws) {
			this._ws.close();
			this._ws = null;
	 ...
	 * 销毁
	 */
	XwRtspPlayer.prototype.destroy = function() {
	 ...
	 * Media数据管理
	 */
	var Mse = function(xwRtspPlayer, type, el) {
		this._xwRtspPlayer = xwRtspPlayer;
		this._type = type;
		this._el = el;
	...
	Mse.prototype._onUpdateend = function() {
		...
		var el = this._el,
			...
			xwRtspPlayer = this._xwRtspPlayer;
		try {
	    	if (!this._resPlayIng && sourceBuffer && !sourceBuffer.updating && mediaSource.readyState == "open") {
	    	    if (sourceBuffer.buffered.length>0) {
	    	    	if (this._isVideo && !this._isSendPlayEventd) {
	    	    		...
	    	    		xwRtspPlayer._dispatchEvent(EventNames.play);
	    	    	}
	    	    	var newPlayTime = sourceBuffer.buffered.end(0);
	                //var newPlayTime2 = parseInt((sourceBuffer.buffered.end(0) - xwRtspPlayer._bufferDelay) * 10 / 10);
	                var dTime = newPlayTime - el.currentTime;
	                if (dTime > xwRtspPlayer._bufferDelayOffect && newPlayTime > 0) {
	        	    	newPlayTime = newPlayTime - xwRtspPlayer._bufferDelay;
	        	    	if (el.currentTime<newPlayTime) {
		                	var now = (new Date()).getTime();
		        	    	if (this._setCurrentTimeH && this._setCurrentTimeH + xwRtspPlayer._setCurrentTimeInterval > now) {
		        	    		return;
		        	    	}this._setCurrentTimeH = now;
	        	    		Log$1.log("xwRtspPlayer[" + xwRtspPlayer._id + "]." + this._type + ".currentTime:from " + el.currentTime + " to " + newPlayTime);
	        	    	}
	                	//console.log();
	...
	Mse.prototype._cleanupBuffer = function() {
	    ...
	    if (isCleand && sourceBuffer && sourceBuffer.buffered.length && !sourceBuffer.updating) {
	        ...
	        var removeEnd = endBuffered - xwRtspPlayer._bufferDuration;

	        if (removeEnd > 0 && bufferedDuration > xwRtspPlayer._bufferDuration && currentPlayTime > startBuffered && currentPlayTime > removeEnd) {
	            try {
	...
	if (videojs) {
		...
		var MainXwRtspPlayer = function () {};
		MainXwRtspPlayer.prototype.initPlayer = function(source, tech, techOptions) {
		    ...
		    _player_ = player._player_ = new XwRtspPlayer(source.src, el, player.options_.xwRtspPlayerConfig, player.el_);
		...
		MainXwRtspPlayer.prototype.handleSource = function(source, tech, techOptions) {
		...
		var LiveSourceHandler = function () {
			this._handle_ = new MainXwRtspPlayer();
		};
		...
		LiveSourceHandler.prototype.canHandleSource = function(source) {
		    ...
		    return XwRtspPlayer.isSupported && this.canPlayType(type);
		}
